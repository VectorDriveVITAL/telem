...
from ..models import ServiceIngestReading, ServiceStatus, BuoyIngestReading, BuoyStatus, BuoyHeartbeat
...
@router.post("/services/{name}", response_model=ServiceStatus, summary="Ingest one reading for a service")
def ingest_service(name: str = Path(..., examples=["telemetry-gateway"]), body: ServiceIngestReading = ...):
    ...
    try:
        state = generator.ingest_service(name, body.latency_ms, body.rps, body.error_rate, body.time)
    except KeyError:
        raise HTTPException(404, f"unknown service: {name} - register it first with POST /api/services")
    return _service_to_status(state)


@router.post("/services/{name}/batch", response_model=ServiceStatus, summary="Ingest a batch of readings for a service")
def ingest_service_batch(name: str = Path(..., examples=["telemetry-gateway"]), body: list[ServiceIngestReading] = ...):
...
@router.post("/buoys/{buoy_id}", response_model=BuoyStatus, summary="Ingest one reading for a buoy")
def ingest_buoy(buoy_id: str = Path(..., examples=["buoy-01"]), body: BuoyIngestReading = ...):
    ...
    try:
        ...
        raise HTTPException(404, f"unknown buoy: {buoy_id} - register it first with POST /api/buoys")
    return _buoy_to_status(state)


@router.post("/buoys/{buoy_id}/batch", response_model=BuoyStatus, summary="Ingest a batch of readings for a buoy")
def ingest_buoy_batch(buoy_id: str = Path(..., examples=["buoy-01"]), body: list[BuoyIngestReading] = ...):
...
@router.post("/buoys/{buoy_id}/heartbeat", response_model=BuoyStatus, summary="Buoy check-in with no sensor data")
def heartbeat(buoy_id: str = Path(..., examples=["buoy-01"]), body: BuoyHeartbeat = BuoyHeartbeat()):
